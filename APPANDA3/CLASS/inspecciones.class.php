...
class Inspecciones{
	...
	public function __construct($_IS_lector=NULL,
		...
		$this->id = $_id;
		$this->leclector = $_leclector;
		$this->lecinspector = $_lecinspector;
		$this->obsev = $_obsev;
	...
	$sql = "INSERT INTO inspecciones VALUES (:islect, :issector, :isruta, :issec, :isdec, :fecha,  :fechaO, :isgrupo, :id, :leclector, :lecinspector, :obsev)";
	...
	$sth->execute(array(
		...
		":id" => $this->id,
		":leclector"=> $this->leclector,
		":lecinspector"=> $this->lecinspector,
		":obsev" => $this->obsev
...
public function update(){
	$pdo= new coneccion();
	$pdo->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "UPDATE inspecciones SET in_codigo_lector=:islect, in_sector=:issector, in_ruta=:isruta, in_secuencia=:issec, in_descripcion=:isdec, in_fechaOrigen=:fecha,  in_fecha=:fechaO, in_grupo=:isgrupo, leclector=:leclector, lecinspector=:lecinspector, obsev=:obsev WHERE idINSP=$this->id";
	$sth = $pdo->db->prepare($sql);
	$sth->setFetchMode(PDO::FETCH_CLASS, "inspecciones");
	$sth->execute(array(
		":islect" => $this->islector,
		...
		":isgrupo" => $this->isgrupo,
		":leclector"=> $this->leclector,
		":lecinspector"=> $this->_lecinspector,
		":obsev" => $this->obsev		
...
";
	...
	public function getcondiario()
	{
		...
		$pdo = new coneccion();
		$pdo->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * from grupo, inspecciones, sector,ruta, lectorAvisador where id_grupo=in_grupo and in_fechaOrigen='$mesactual'   and id=in_sector and Rid=in_ruta and codiglector=in_codigo_lector AND in_codigo_lector like '%$this->islector%' ;  
";
		$sth = $pdo->db->prepare($sql);
		$sth->setFetchMode(PDO::FETCH_CLASS, "inspecciones");
		$sth->execute();
...
public function getcondiario2()
	{
		...
		$pdo = new coneccion();
		$pdo->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * from grupo, inspecciones, sector,ruta, lectorAvisador where  id=in_sector and Rid=in_ruta  and in_codigo_lector like '%$this->islector%' ;
";
		$sth = $pdo->db->prepare($sql);
		$sth->setFetchMode(PDO::FETCH_CLASS, "inspecciones");
		$sth->execute();
