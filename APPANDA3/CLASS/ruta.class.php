...
public function getcon(){
	...
	public function getcondiario2(){
		...
		$pdo = new coneccion();
		$pdo->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT agencia.id_agencia, grupo.id_grupo, sector.id_sector, ruta.Rid, ruta.n_ruta,  ruta.total_usuarios, ruta.r_hasta, ruta.r_desde, ruta.Rfecha FROM agencia, grupo, sector, ruta WHERE  id_agencia=g_id_agencia AND id_grupo=s_id_grupo AND id=r_id_sector order by id_grupo" ;
		$sth = $pdo->db->prepare($sql);
		$sth->setFetchMode(PDO::FETCH_CLASS, "ruta");
		$sth->execute();
		$rows = $sth->fetchAll(PDO::FETCH_CLASS);
	...
	public function getrutas2(){
		
		$pdo = new coneccion();
		$pdo->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT  * FROM  sector, ruta WHERE  s_id_grupo='$this->id' AND id=r_id_sector"  ;
		$sth = $pdo->db->prepare($sql);
		$sth->setFetchMode(PDO::FETCH_CLASS, "ruta");
		$sth->execute();
		$rows = $sth->fetchAll(PDO::FETCH_CLASS);
