...
return $sth->rowCount();
	...
	public function getdiario2()
	{
		...
		$pdo= new coneccion();
		$pdo->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * from asignacion, ruta,  grupo, lectoravisador, sector WHERE  ar_codigolec=codiglector and ar_grupo=id_grupo  and ar_id_sector=id and ar_nruta=Rid and  codiglector like'%$this->rutfecha%' order by fecha_asig desc";
		$sth = $pdo->db->prepare($sql);
		$sth->setFetchMode(PDO::FETCH_CLASS, "asignacion");
		$sth->execute();
		$rows = $sth->fetchAll(PDO::FETCH_CLASS);
...
public function getdiario3()
	{
		...
		$pdo= new coneccion();
		$pdo->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * from asignacion, ruta,  grupo, lectoravisador, sector WHERE  ar_codigolec=codiglector and ar_grupo=id_grupo  and ar_id_sector=id and ar_nruta=Rid and  limit '$this->rutfecha', '$this->periodo' ";
		$sth = $pdo->db->prepare($sql);
		$sth->setFetchMode(PDO::FETCH_CLASS, "asignacion");
		$sth->execute();
		$rows = $sth->fetchAll(PDO::FETCH_CLASS);
	...
	public function getdiario4()
	{
	
		$pdo= new coneccion();
		$pdo->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * from asignacion, ruta,  grupo, lectoravisador, sector WHERE  ar_codigolec=codiglector and ar_grupo=id_grupo  and ar_id_sector=id and ar_nruta=Rid  and  codiglector like'%$this->rutfecha%' and id_sector!=301 and codiglector!=0000 order by fecha_asig desc ";
		$sth = $pdo->db->prepare($sql);
		$sth->setFetchMode(PDO::FETCH_CLASS, "asignacion");
		$sth->execute();
		$rows = $sth->fetchAll(PDO::FETCH_CLASS);
