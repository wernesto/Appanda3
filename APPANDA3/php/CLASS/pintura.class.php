...
public function getbuscar()
	...
	public function getbuscardia2()
	{
		...
		$pdo = new coneccion();
		$pdo->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * from   lectorAvisador,grupo, ruta_pintada, ruta, sector where id_grupo=Pin_id_grupo and codiglector=rp_cod_lector and  id=rp_id_sector and Rid=rp_id_ruta and   codiglector  like '%$this->lector%' ORDER BY RPfecha_origen desc ";
		$sth = $pdo->db->prepare($sql);
		$sth->setFetchMode(PDO::FETCH_CLASS, "ruta_pintada");
		$sth->execute();
		$rows = $sth->fetchAll(PDO::FETCH_CLASS);
